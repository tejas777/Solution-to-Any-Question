  ...
  <nav class="navbar navbar-default navbar-fixed-top" >
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav navbar-right">
        ...
        <li><a href="#"><span class="fa fa-question-circle-o"></span> Your Questions</a></li>
...
<?php
    ...
    $sql = "SELECT * FROM question_bank WHERE User = '$Name'";
    ...
    if (mysql_num_rows($result) > 0) {
        ...
        while ($doc = mysql_fetch_array($result)) {
          ...
          echo '<div style="margin-top:-45px;margin-left:98%;" class="dropdown">
                      <span dropdown-toggle" data-toggle="dropdown" style="font-size:30px;text-decoration:none;"><a href="">...</a></span>
                      <ul class="dropdown-menu">
                        <li><a href="Edit.php?Ques_Id='.$Question_Id.'">Edit</a></li>
                        <li><a href="Delete.php?Ques_Id='.$Question_Id.'&Name='.$Name.'">Delete</a></li>
                      </ul>
                    </div></span>';
?>
...
<?php }
    ...
    $conn = new MongoClient();            //Connect To MongoDB
        if($conn){
            ...
            $result=$collection_question->find(['Name' => "$Name"])->sort(array('Time_Of_Posting'=> -1));
            ...
            echo '<div class="main_box" data-spy="affix">';         
              foreach ($result as $result_question) {
                ...
                //$result_question=$collection_question->findOne(['_id' => new MongoID($Question_Id)]);
                    echo '<span class="middleware_category"> Category : <a href="view_ques_category.php?Category='.$Category.'">'.$result_question["Category"].'</a><i> '.time_elapsed_string($result_question["Time_Of_Posting"]).'</i>
                    
                    <a href="View_Ques.php?Question_Id='.$Question_Id.'"><h1 style="font-family: labo, serif;">'.$result_question["Question"].'</h1></a>
                    <div style="margin-top:-45px;margin-left:98%;" class="dropdown">
                      <span dropdown-toggle" data-toggle="dropdown" style="font-size:30px;text-decoration:none;"><a href="">...</a></span>
                      <ul class="dropdown-menu">
                        <li><a href="Edit.php?Ques_Id='.$Question_Id.'">Edit</a></li>
                        <li><a href="Delete.php?Ques_Id='.$Question_Id.'&Name='.$Name.'">Delete</a></li>
    ...
    ?>
                    <img src="/The_Solution/Profile_Pic/<?php echo $result_question['Profile_Pic'];?>" width="42px" height="42px" style="border-radius:50%"//>
    <?php
                    echo '<span style="padding:10px;color: #403f3f ;"><a href="view_profile.php?Name='.$result_question["Name"].'">'.$result_question["Name"].'</a></span>';
    ...
    <?php echo "<br/><br/>";
              ...
              $collection_answers=$database->answer_bank;       //Connect to user_data collection
                $result_answer = $collection_answers->find(['Question_Id' => "$Question_Id"]);
  ...
  ?>
            ...
            <a href="like.php?User=<?php echo $Name;?>&Answer_Id=<?php echo $Answer_Id;?>" class="fa fa-thumbs-up social">
                
  <?php
                    $collection_likes=$database->likes; 
                            
                    $count = $collection_likes->count(['Answer_Id' => "$Answer_Id"]);
                    echo $count;
  ?> 
            </a>
&nbsp;&nbsp;
  ...
  <?php 
                        ...
                        $count = $collection_dislikes->count(['Answer_Id' => "$Answer_Id"]);
                        echo $count;
  ?>
                  </a>  
  <?php
            ...
            echo "<br/><br/><h4 style=color:#b30000;>Question is not yet Answered</h4>";
          }
